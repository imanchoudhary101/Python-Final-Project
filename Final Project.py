...
def message(msg):
    ...
    window.blit(msg,[win_widht/18 ,win_height/3])
def game_loop():
    gameOver = False
    gameClose = False
    x1 = win_widht/2
    ...
    snake_length_list = []
    sanke_lenght = 1
    foodx = round(random.randrange)(0,win_widht-snake/10.0)*10.0
    ...
    if x1 > win_widht or x1< 0 or y1 > win_height or y1< 0: 
        gameClose = True
    x1 += x1_change 
    y1 += y1_change 
    window.fill(grey)
